...
class ControllerSaleOrder extends Controller {
	...
	public function index() {
			...
			$mysql = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "partial_payment'");
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if (!isset($result['date_next_pay'])) {
			$result['date_next_pay'] = @$result['date_modified'];
			} else {
			$result['date_next_pay'] = $result['date_next_pay'];
			}
			if (!isset($result['date_reminder'])) {
			$result['date_reminder'] = @$result['date_modified'];
			} else {
			$result['date_reminder'] = $result['date_reminder'];
			}
			$this->load->language('sale/order');
			...
			$data['orders'][] = array(
			   ...
			   'partial_period'    => @$text_partial_period,
			   'date_next_pay2'    => strtotime($result['date_next_pay']),
			   'date_reminder2'    => strtotime($result['date_reminder']),
			   'date_next_pay1'    => $result['date_next_pay'],
			   'date_reminder1'    => $result['date_reminder'],
			   'date_now'    => time(),
			   'date_next_pay'    => date($this->language->get('date_format_short'), strtotime($result['date_next_pay'])),
			   'date_reminder'    => date($this->language->get('date_format_short'), strtotime($result['date_reminder'])),           
...
$data['sort_payment_pending'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.pending_total' . $url, true);
			$data['sort_partial_period'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=pp.partial_period' . $url, true);
			$data['sort_date_next_pay'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=pp.date_next_pay' . $url, true);
			$data['sort_date_reminder'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=pp.date_reminder' . $url, true);
...
$this->load->language('extension/total/partial_payment_total');
			...
			$data['text_date_next_pay'] = $this->language->get('text_date_next_pay');
			$data['text_date_reminder'] = $this->language->get('text_date_reminder');
			$data['text_partial_period'] = $this->language->get('text_partial_period');
			$data['text_partial_percent'] = $this->language->get('text_partial_percent');
