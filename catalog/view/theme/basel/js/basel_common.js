...
$(document).ready(function () {
    ...
    // Highlight wishlist
    $("#wl-ids span").each(function () {
        var id = $(this).attr('id').replace('wl-', '');
        $(".product_" + id).find(".icon-star").addClass('icon-star-hovered').parent().css("transform", "scale(1,1)").attr("onclick", "wishlist.remove(" + id + ");").attr('data-title', "Remove from Wish List").find('span.title-wl').text("Remove from Wish List");
    });
    ...
    //Highlight links in account
    var cur_href = $(location).attr('href');
    $("body[class^='account-'] #page-title").text($(".theiaStickySidebar .widget a[href*='" + cur_href + "']").addClass("current").text());
    $(".theiaStickySidebar .widget a").each(function (e) {
        if (cur_href.indexOf($(this).attr('href')) >= 0) {
    ...
    //Login please
    $patch = $("#patch").removeAttr('id');
    if ($patch.length > 0) $(".MagicToolboxSelectorsContainer").remove();
    $patch.clone().show().appendTo(".single-product .image");
    $patch.clone().show().appendTo("#content .main-image");
});
...
var quickview = function (product_id) {
    $.ajax({
        ...
        success: function (html) {
            ...
            //Login pls
            $patch.clone().show().appendTo("#gallery");
        }
    });
}
...
var wishlist = {
    'add': function (product_id) {
        $.ajax({
            ...
            success: function (json) {
                ...
                $(".product_" + product_id).find(".icon-star").addClass('icon-star-hovered').parent().css("transform", "scale(1,1)").attr("onclick", "wishlist.remove(" + product_id + ");").attr('data-original-title', 'Remove from Wish List').find(".title-wl").text("Remove from Wish List");
                ;
            },
            error: function (xhr, ajaxOptions, thrownError) {
                alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
    ...
    'remove': function (product_id) {
        $.ajax({
            url: 'index.php?route=extension/basel/basel_features/remove_from_wishlist',
            type: 'post',
            data: 'product_id=' + product_id,
            dataType: 'json',
            success: function (json) {
                ...
                $('.wishlist-total span').html(json['total']);
                $('.wishlist-counter').html(json['total_counter']);
                $(".product_" + product_id).find(".icon-star").removeClass('icon-star-hovered').parent().css("transform", "").attr("onclick", "wishlist.add(" + product_id + ");").attr('data-original-title', 'Add to Wish List').find(".title-wl").text("Add to Wish List");
            },
            error: function (xhr, ajaxOptions, thrownError) {
                alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
            }
