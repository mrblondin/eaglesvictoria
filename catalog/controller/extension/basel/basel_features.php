...
// Lang curr title 
    public function lang_curr_title()
    {
        ...
        foreach ($results2 as $result2) {
            if ($result2['status']) {
                //if($i < 1){//Temporary added to hide languages, should be deleted
                    $languages[] = array(
                        'name' => $result2['name'],
                        'code' => $result2['code']
                    );
        ...
        foreach ($languages as $language) {
            if (count($languages) > 1) {
                if ($language['code'] == $lang_code) {
                    $output .= '<span>' . '<img style="bottom:1px; position:relative" src="catalog/language/' . $lang_code . '/' . $lang_code . '.png" alt="{{ ' . $language['name'] . ' }}" title="{{ ' . $language['name'] . ' }}"> ' . $language['name'] . '</span>';
                }
            }
        }
        // Currency
