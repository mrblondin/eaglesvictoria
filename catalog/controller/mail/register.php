...
class ControllerMailRegister extends Controller {
	public function index(&$route, &$args, &$output) {
		...
		$data['text_welcome'] = sprintf($this->language->get('text_welcome'), html_entity_decode($args[0]['firstname'], ENT_QUOTES, 'UTF-8'));
		$data['text_login'] = $this->language->get('text_login');
		$data['text_approval'] = $this->language->get('text_approval');
		$data['text_service'] = $this->language->get('text_service');
		$data['text_thanks'] = $this->language->get('text_thanks');
        $data['c_name'] = $args[0]['firstname'];
        $data['c_mail'] = $args[0]['email'];
        ...
        $custverf_token = substr(md5($args[0]['email']), 0, 20);//substr(md5(mt_rand()), 0, 20);
        $this->session->data['custverf_token'] = $custverf_token;
        //$this->db->query("INSERT INTO " . DB_PREFIX . "custverf set token='" . $this->db->escape($custverf_token) . "', customer_id='" . (int)$args[0]['customer_id'] . "', confirmed=0 ");
        $confirm_link = $this->url->link('extension/verifyemail', 'token=' . $custverf_token);
        $data['custverlink'] = '<a href="'.$confirm_link.'">'.$confirm_link.'</a>';
