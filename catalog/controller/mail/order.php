...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		foreach ($order_totals as $order_total) {
            ...
            if ($order_total['code'] == 'total') $total_amount = $order_total['value'];
            //Доп инфа для паршиал пэймент
            if($order_total['code'] == 'partial_payment_total'){
                $data['totals'][] = array(
                    'title' => "Pending Amount",
                    'text'  => $this->currency->format($total_amount - $order_total['value'], $order_info['currency_code'], $order_info['currency_value']),
                );

                $data['totals'][] = array(
                    'title' => "Due Date",
                    'text'  => date('M d, Y', strtotime("+90 day")),
                );

                if ($data['comment'] == '') {
                    $data['comment'] = "Thank you for the purchase!<br/>
The rest of the sum should be paid in 90 days, total cost invoice will be sent by e-mail.<br/>
While you are in the process of payments the chosen item is reserved for you. The item will be sent after you complete your layaway payment and pay its total cost. ";
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
            ...
            $data['payment_method'] = substr($order_info['payment_method'], strpos($order_info['payment_method'], "<img") );
            $data['shipping_method'] = $order_info['shipping_method'];
            $data['email'] = $order_info['email'];
            $data['telephone'] = $order_info['telephone'];
            $data['ip'] = $order_info['ip'];
