...
class ModelExtensionShippingXshippingpro extends Model {
	...
	function getQuote($address) {
		...
		foreach($methods as $single_method) {
			...
			if (!$_flags['coupon'] && $xshippingpro['rate_type'] == 'total_coupon') {
            ...
            if ($multi_category && $xshippingpro['multi_category'] =='any' && $xshippingpro['category'] != 1) {
			...
			if ($is_product_specified || $xshippingpro['method_specific']) {
				...
				foreach($cart_products as $product) {
					...
					if (($xshippingpro['location_rule']==2 || $xshippingpro['location_rule']==3 || $xshippingpro['location_rule']==4 || $xshippingpro['location_rule']==5 || $xshippingpro['location_rule']==6 || $xshippingpro['location_rule']==7) && !in_array($product['location'],$applicable_locations)) {   
						continue;
					}
					...
					$product['weight_class_id'] = $product['weight_class_id'] ? $product['weight_class_id'] : $this->config->get('config_weight_class_id');
		...
		if ($xshippingpro['rate_type']=='flat') {
			...
			if ($xshippingpro['rate_final']=='cumulative') {
			 ...
			 if ($rate_min && $rate_min > $cost && $modifier_allowed) {
				$cost = $rate_min;
			 }

			 if ($rate_max && $rate_max < $cost && $modifier_allowed) {
				$cost = $rate_max;
			 }
			...
			if ($xshippingpro['equation']) {
				...
				$placholder = array('{cartTotal}','{cartQnty}','{cartWeight}', '{shipping}', '{modifier}', '{volume}', '{noOfCategory}', '{noOfManufacturer}', '{noOfLocation}');

				$eq_total = $xshippingpro['method_specific'] ? $method_total : $cart_total;
				$eq_qnty = $xshippingpro['method_specific'] ? $method_quantity : $cart_quantity;
				$eq_weight = $xshippingpro['method_specific'] ? $method_weight : $cart_weight;
				$eq_volume = $xshippingpro['method_specific'] ? $method_volume : $cart_volume;

				$replacer = array($eq_total,$eq_qnty,$eq_weight,$eq_shipping,$eq_modifier, $eq_volume, $noOfCategory, $noOfManufacturer, $noOfLocation);
		...
		$method_desc= (isset($xshippingpro['desc'][$language_id]) && $xshippingpro['desc'][$language_id] && !$is_admin) ? '<span style="color: #999999;font-size: 11px;display:block" class="x-shipping-desc">'.$xshippingpro['desc'][$language_id].'</span>' : '';
